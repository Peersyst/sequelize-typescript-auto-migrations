...
import _, {clone, each, extend} from "lodash";
...
export const sortActions = function (actions) {
    ...
    const uniqueActions = _.uniq(actions.map(JSON.stringify)).map((a: string) => JSON.parse(a));
    ...
    for (let k = 0; k <= uniqueActions.length; k++)
        for (let i = 0; i < uniqueActions.length; i++) {
            if (!uniqueActions[i].depends) continue;
            if (uniqueActions[i].depends.length === 0) continue;

            let a = uniqueActions[i];

            for (let j = 0; j < uniqueActions.length; j++) {
                if (!uniqueActions[j].depends) continue;
                if (uniqueActions[j].depends.length === 0) continue;

                let b = uniqueActions[j];

                if (a.actionType != b.actionType) continue;

                if (b.depends.indexOf(a.tableName) !== -1 && i > j) {
                    let c = uniqueActions[i];
                    uniqueActions[i] = uniqueActions[j];
                    uniqueActions[j] = c;
                }
            }
        }
